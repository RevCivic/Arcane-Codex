    ...
    <div className="max-w-3xl">
      ...
      <div className="card-arcane rounded-lg p-6" style={{  }}>
        <div className="flex items-start justify-between mb-4">
          <div>
            <h1 className="text-3xl font-bold mb-2" style={{ color: '#e8eef7' }}>{item.name}</h1>
            {item.category && (
        ...
        <dl className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {item.description && (
            <div className="sm:col-span-2">
              ...
              <dd className="text-sm leading-6" style={{ color: '#e8eef7' }}>{item.description}</dd>
            </div>
          )}
          {item.effect && (
            <div className="sm:col-span-2">
              <dt className="text-xs uppercase tracking-wider mb-1" style={{ color: '#d97706' }}>Effect</dt>
              <dd className="text-sm leading-6 p-3 rounded italic" style={{ color: '#a78bfa', backgroundColor: '#0d0d15', border: '1px solid #2a2a3e' }}>{item.effect}</dd>
            </div>
          )}
          {item.location && (
            <div>
              <dt className="text-xs uppercase tracking-wider mb-1" style={{ color: '#d97706' }}>Location</dt>
              <dd className="text-sm" style={{ color: '#e8eef7' }}>{item.location}</dd>
            </div>
          )}
          {item.carrier && (
            <div>
