  ...
  return (
    <div className="max-w-5xl">
      ...
      {typeof result.deduplicated === 'string' && (
        <div className="rounded-lg p-4 mb-4" style={{ backgroundColor: '#111118', border: '1px solid #2d1b69', color: '#e8eef7' }}>
      ...
      {typeof result.pruned === 'string' && (
        <div className="rounded-lg p-4 mb-8" style={{ backgroundColor: '#111118', border: '1px solid #451a03', color: '#f3f4f6' }}>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
        <div className="rounded-lg p-5" style={{ backgroundColor: '#111118', border: '1px solid #2a2a3e' }}>
          <div className="text-xs uppercase tracking-widest mb-2" style={{ color: '#a0a9b8' }}>Total Tags</div>
          <div className="text-3xl font-bold" style={{ color: '#e8eef7' }}>{tags.length}</div>
        </div>
        <div className="rounded-lg p-5" style={{ backgroundColor: '#111118', border: '1px solid #2a2a3e' }}>
          <div className="text-xs uppercase tracking-widest mb-2" style={{ color: '#a0a9b8' }}>Unused Tags</div>
          <div className="text-3xl font-bold" style={{ color: '#f59e0b' }}>{unusedTagCount}</div>
        </div>
        <div className="rounded-lg p-5" style={{ backgroundColor: '#111118', border: '1px solid #2a2a3e' }}>
          <div className="text-xs uppercase tracking-widest mb-2" style={{ color: '#a0a9b8' }}>Duplicate Groups</div>
          <div className="text-3xl font-bold" style={{ color: '#8b5cf6' }}>{duplicateGroups.length}</div>
        </div>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-8">
        <form action={deduplicateTags} className="rounded-lg p-6 space-y-3" style={{ backgroundColor: '#111118', border: '1px solid #2a2a3e' }}>
        ...
        <form action={pruneUnusedTags} className="rounded-lg p-6 space-y-3" style={{ backgroundColor: '#111118', border: '1px solid #2a2a3e' }}>
      ...
      <div className="rounded-lg overflow-hidden" style={{ border: '1px solid #2a2a3e' }}>
        <div className="px-4 py-3" style={{ backgroundColor: '#111118', borderBottom: '1px solid #2a2a3e' }}>
          <h2 className="text-sm uppercase tracking-widest" style={{ color: '#d97706' }}>Case-Insensitive Duplicate Groups</h2>
        </div>
        {duplicateGroups.length === 0 ? (
          <div className="px-4 py-8 text-sm" style={{ color: '#a0a9b8' }}>
            ...
            {duplicateGroups.map((group) => (
              <div key={group.key} className="px-4 py-4">
                <div className="flex items-center justify-between gap-3 mb-2">
                  <span className="text-sm font-semibold" style={{ color: '#e8eef7' }}>{group.key}</span>
                ...
                <div className="flex flex-wrap gap-2">
                  {group.tags.map((tag) => (
                    <span
                      ...
                      style={{ backgroundColor: '#0d0d15', color: '#9ca3af', border: '1px solid #2a2a3e' }}
                    >
                      <span>{tag.name}</span>
