...
export default async function PlacesPage({
  ...
  const places = await prisma.place.findMany({ orderBy: { [sortBy]: sortOrder } })
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', fontFamily: 'Georgia, serif' }}>
            <thead>
              <tr style={{ borderBottom: '2px solid #1f2937', backgroundColor: '#0d0d1a' }}>
                ...
                <th style={thStyle}>
                  <Link href={sortLink(view, sortBy, sortOrder, 'type')} style={{ color: sortBy === 'type' ? '#a78bfa' : '#6b7280', textDecoration: 'none', fontSize: '11px', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em' }}>
                    Type<SortIcon sortBy={sortBy} sortOrder={sortOrder} column="type" />
                  </Link>
                </th>
                <th style={thStyle}>
                  <Link href={sortLink(view, sortBy, sortOrder, 'region')} style={{ color: sortBy === 'region' ? '#a78bfa' : '#6b7280', textDecoration: 'none', fontSize: '11px', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.05em' }}>
